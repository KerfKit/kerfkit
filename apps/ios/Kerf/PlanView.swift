import SwiftUI
import CutModels

// Plan ekranı MVP'si (E4-S3 özü): istatistik kartı + levha sekmeli Canvas diyagramı.
// Pinch-zoom, bayat-sonuç bandı ve 60fps ölçümü tam M-4 oturumunda (docs/13).
struct PlanView: View {
    let result: OptimizeResult
    let store: ProjectStore
    @Environment(\.dismiss) private var dismiss

    var body: some View {
        NavigationStack {
            VStack(spacing: 16) {
                HStack(spacing: 12) {
                    StatCard(title: "Levha", value: "\(result.stats.sheetCount)")
                    StatCard(title: "Fire", value: String(format: "%%%.1f", Double(result.stats.wasteBps) / 100))
                    StatCard(title: "Kesim", value: "\(result.stats.cutCount)")
                }
                .padding(.horizontal)

                if !result.unplaced.isEmpty {
                    Label("\(result.unplaced.count) parça yerleşmedi — stok adedini artırın",
                          systemImage: "exclamationmark.triangle.fill")
                        .font(.callout)
                        .foregroundStyle(DesignTokens.colorAmber400)
                }

                TabView {
                    ForEach(0..<result.stats.sheetCount, id: \.self) { sheet in
                        SheetDiagram(
                            placements: result.placements.filter { $0.sheetIndex == sheet },
                            names: store.partNames,
                            sheetW: store.sheetW, sheetH: store.sheetH)
                            .padding(.horizontal)
                            .padding(.bottom, 40)
                    }
                }
                .tabViewStyle(.page)
            }
            .navigationTitle("Kesim planı")
            .navigationBarTitleDisplayMode(.inline)
            .toolbar {
                ToolbarItem(placement: .confirmationAction) {
                    Button("Kapat") { dismiss() }
                }
            }
            .background(DesignTokens.colorTimber950)
        }
    }
}

struct StatCard: View {
    let title: String
    let value: String

    var body: some View {
        VStack(spacing: 4) {
            Text(value)
                .font(.title2.bold().monospacedDigit())
                .foregroundStyle(DesignTokens.colorAmber500)
            Text(title)
                .font(.caption)
                .foregroundStyle(DesignTokens.colorTimber300)
        }
        .frame(maxWidth: .infinity)
        .padding(.vertical, 12)
        .background(DesignTokens.colorTimber900, in: RoundedRectangle(cornerRadius: 12))
    }
}

// Diyagram dili docs/12 §5 özü: koyu levha zemini + amber parçalar + etiketler.
// Motor orijini sol-alt; Canvas sol-üst — y ekseni çevrilir.
struct SheetDiagram: View {
    let placements: [Placement]
    let names: [String: String]
    let sheetW: Units
    let sheetH: Units

    var body: some View {
        Canvas { ctx, size in
            let scale = min(size.width / CGFloat(sheetW), size.height / CGFloat(sheetH))
            let w = CGFloat(sheetW) * scale, h = CGFloat(sheetH) * scale
            let ox = (size.width - w) / 2, oy = (size.height - h) / 2

            ctx.fill(Path(CGRect(x: ox, y: oy, width: w, height: h)),
                     with: .color(DesignTokens.colorTimber800))

            for p in placements {
                let rect = CGRect(
                    x: ox + CGFloat(p.x) * scale,
                    y: oy + h - CGFloat(p.y + p.h) * scale,
                    width: CGFloat(p.w) * scale,
                    height: CGFloat(p.h) * scale)
                ctx.fill(Path(roundedRect: rect.insetBy(dx: 0.5, dy: 0.5), cornerRadius: 2),
                         with: .color(DesignTokens.colorAmber500))
                let name = names[p.partId] ?? p.partId
                let label = Text("\(name)\(p.rotated ? " ⤾" : "")")
                    .font(.system(size: 11, weight: .semibold))
                    .foregroundStyle(DesignTokens.colorTimber950)
                if rect.width > 40 && rect.height > 16 {
                    ctx.draw(label, at: CGPoint(x: rect.midX, y: rect.midY))
                }
            }
        }
        .aspectRatio(CGFloat(sheetW) / CGFloat(sheetH), contentMode: .fit)
        .accessibilityLabel("Kesim diyagramı: \(placements.count) parça yerleşimi")
    }
}
